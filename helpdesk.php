...
// F1.2 — was commented out; re-enabled and fixed the trailing-space typo
// ("HDU_LOGO " → "HDU_LOGO"). Templates already reference the space-less
// constant, so any theme override was silently ignored before.
...
$_session = e107::getSession();
if ($_session->is('R1'))          { $R1        = $_session->get('R1'); }
if ($_session->is('hdu_goto'))    { $hdu_goto  = $_session->get('hdu_goto'); }
// NOTE: hdu_savemsg is a message, not a filter — do not overwrite $R1 with it.
// if ($_session->is('hdu_savemsg')) { /* consumed later */ }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST')
{
    ...
    $id          = (int) ($_POST['id']          ?? 0);
    $R1          = $_POST['R1']                 ?? $R1;
    $hdu_goto    = (int) ($_POST['goto']        ?? 0);
}
elseif (e_QUERY)
{
    $tmp         = explode('.', e_QUERY);
    $from        = (int) ($tmp[0]               ?? 0);
    $hdu_aaction = $tmp[1]                      ?? null;
    $id          = (int) ($tmp[2]               ?? 0);
    // $R1 = $tmp[3] ?? $R1;
}
if ((int) $hdu_goto === 0)
...
// F1.1b — variables locales acumuladas por el flujo action-switch de más abajo.
// Sin inicializar generaban `Undefined variable` warnings en PHP 8.
$hdu_savemsg = '';
$sqlmsg      = '';
$filter      = '';
$hdu_text    = '';
